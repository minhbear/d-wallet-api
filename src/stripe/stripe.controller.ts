...
export class StripeController {
  ...
  @Post()
  async handleStripeWebhook(@Req() req: RawBodyRequest<Request>): Promise<void> {
    const handleWebhookEvent = {
      [WebhookEventStripe.PAYMENTSUCCEEDED]: async (id: string) => {
        const paymentIntent: Stripe.PaymentIntent = await this.stripeService.getPaymentIntent(id);
        await this.stripeService.handlePaymentIntentSuccess(
          paymentIntent,
        );
      },
      [WebhookEventStripe.PAYMENTCANCELED]: async (id: string) => {
        const paymentIntent: Stripe.PaymentIntent = await this.stripeService.getPaymentIntent(id);
        await this.stripeService.handlePaymentIntentFail(
          paymentIntent,
        );
      }
    }
